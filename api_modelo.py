...
def predict():
    '''
    Endpoint protegiso por token para obter predição.
    Corpo (JSON):
    {
    "sepal_length": 5.1,
    "sepal_width": 3.5,
    ...
    try:
        ...
        return jsonify({"error": "Dados inválidos, verifique parãmetros"}), 400
    
    # Verificar se já está em cache
    features = (sepal_length, sepal_width, petal_length, petal_width)
    if features in prediction_cache:
        logger.info('Cache hit para %s', features)
        predict_class = prediction_cache[features]
    else:
        # Rodar modelo
        ...
        # Armazenar no cache
        prediction_cache[features] = predict_class
    ...
    db.commit()
    db.close()
    return jsonify({"predicted_class": predict_class})
